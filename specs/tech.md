...
> к нашему RU-only репозиторию НЕ относятся. Наш стек/структура — в `CLAUDE.md` (раздел «Карта файлов»
> + «Архитектура»). Чекбоксы `[x]` и «CRITICAL/hard-enforced» здесь — про чужой проект, не про нас.
